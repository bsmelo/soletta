...
enum sol_lwm2m_bootstrap_event {
    /**
     * Indicates that a server performed Bootstrap Write to the client.
     */
    SOL_LWM2M_BOOTSTRAP_EVENT_WRITE,
    /**
     * Indicates that a server performed Bootstrap Delete to the client.
     */
    SOL_LWM2M_BOOTSTRAP_EVENT_DELETE,
    /**
 ...
 * This means that every time a LWM2M client requests a bootstrap
...
int sol_lwm2m_bootstrap_server_del_request_monitor(struct sol_lwm2m_bootstrap_server *server,
    void (*sol_lwm2m_server_bootstrap_request_cb)(void *data,
    struct sol_lwm2m_bootstrap_server *server,
    struct sol_lwm2m_bootstrap_client_info *bs_cinfo),
    const void *data);
 ...
 * This means that every time a LWM2M bootstrap server performs a Bootstrap Write,
 * Bootstrap Delete or Bootstrap Finish
 ...
 * @return 0 on success, -errno on error.
 * @see sol_lwm2m_client_del_bootstrap_monitor()
 */
int sol_lwm2m_client_add_bootstrap_monitor(struct sol_lwm2m_client *client,
 ...
 * @see sol_lwm2m_client_add_bootstrap_monitor()
 */
int sol_lwm2m_client_del_bootstrap_monitor(struct sol_lwm2m_client *client,
    void (*sol_lwm2m_client_bootstrap_event_cb)(void *data,
    struct sol_lwm2m_client *client,
    enum sol_lwm2m_bootstrap_event event),
    const void *data);
...
/**
 * @brief Writes an object, object instance or resource through the Bootstrap Interface.
 *
 * @param server The LWM2M bootstrap server.
...
int sol_lwm2m_bootstrap_server_write(struct sol_lwm2m_bootstrap_server *server,
    ...
    const char *path, enum sol_coap_response_code response_code),
    const void *data);
