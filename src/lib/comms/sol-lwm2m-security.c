    ...
    char *psk, size_t psk_len)
{
    ...
    struct sol_lwm2m_key_pair *key_pair;
    uint16_t i;
    ...
    SOL_VECTOR_FOREACH_IDX (known_psks, key_pair, i) {
        if (sol_str_slice_eq(sol_str_slice_from_blob(key_pair->public_key_or_id), id)) {
            if (key_pair->secret_key->size != SOL_DTLS_PSK_KEY_LEN) {
                SOL_WRN("The PSK '%.*s' is %zu-bytes long; expecting a %d-bytes long PSK",
                    SOL_STR_SLICE_PRINT(sol_str_slice_from_blob(key_pair->secret_key)),
                    key_pair->secret_key->size, SOL_DTLS_PSK_KEY_LEN);
                return -EINVAL;
            }

            memcpy(psk, key_pair->secret_key->mem, key_pair->secret_key->size);
...
{
    //TODO: FIXME: Based on what information can I know which Client I'm looking for here?
    char s_psk_id[] = "Client Key ID";
    size_t s_psk_id_len = sizeof(s_psk_id) - 1;

    //If the caller expects a PSK_ID_LEN less than 16 bytes
    if (id_len < SOL_DTLS_PSK_ID_LEN)
        return -ENOBUFS;

    if (s_psk_id_len > SOL_DTLS_PSK_ID_LEN) {
        SOL_WRN("The PSK ID '%s' is %zu-bytes long;"
            " expecting a PSK ID at most %d-bytes long",
            s_psk_id, s_psk_id_len, SOL_DTLS_PSK_ID_LEN);
        return -EINVAL;
    }

    memcpy(id, s_psk_id, s_psk_id_len);

    return (ssize_t)s_psk_id_len;
}
    ...
    enum sol_lwm2m_security_mode sec_mode)
{
    ...
    case LWM2M_CLIENT:
        ...
        socket_dtls = sol_coap_server_get_socket(
            ((struct sol_lwm2m_client *)entity)->coap_server);
        break;
    case LWM2M_SERVER:
    ...
    //TODO: Use sec_mode to choose the callbacks
    // Currently assuming Pre-Shared Key only
