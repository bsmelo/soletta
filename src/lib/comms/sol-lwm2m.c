    ...
    const struct sol_network_link_addr *cliaddr)
{
    ...
    // TODO: Do we need to keep clients like this?
/*
    r = sol_ptr_vector_append(&server->clients, cinfo);
    SOL_INT_CHECK_GOTO(r, < 0, err_exit_del_client);
 */
...
sol_lwm2m_bootstrap_server_new(uint16_t port)
{
...
SOL_API int
sol_lwm2m_client_add_bootstrap_monitor(struct sol_lwm2m_client *client,
    void (*cb)(void *data,
    struct sol_lwm2m_client *client,
    enum sol_lwm2m_bootstrap_event event),
    const void *data)
...
SOL_API int
sol_lwm2m_client_del_bootstrap_monitor(struct sol_lwm2m_client *client,
    void (*cb)(void *data,
    struct sol_lwm2m_client *client,
    enum sol_lwm2m_bootstrap_event event),
    const void *data)
    ...
    const struct sol_str_slice payload)
{
    ...
    //If write_resource is not NULL then write_tlv is guaranteed to ve valid as well.
    ...
    const struct sol_network_link_addr *server_addr)
{
    ...
    SOL_INF("Received Bootstrap Reply");
    ...
    struct server_conn_ctx *conn_ctx)
{
    ...
    r = sol_coap_send_packet_with_reply(client->coap_server,
        pkt,
        sol_vector_get_no_check(&conn_ctx->server_addr_list,
        conn_ctx->addr_list_idx), bootstrap_reply, conn_ctx);
    sol_buffer_fini(&query);
    return r;
    ...
    struct sol_lwm2m_bootstrap_client_info *client)
{
    ...
    SOL_DBG("Sending Bootstrap Finish to LWM2M Client");
...
sol_lwm2m_client_start(struct sol_lwm2m_client *client)
{
    ...
    if (!has_server) {
        ...
        r = read_resources(client, ctx, sol_vector_get_no_check(&ctx->instances, bootstrap_account_idx), res, 3,
        ...
        // TODO: Create unknown resources to handle Bootstrap Write and Bootstrap Delete
        ...
        r = -ENOMEM;
        SOL_NULL_CHECK_GOTO(conn_ctx, err_clear_3);
