    ...
    enum sol_lwm2m_bootstrap_event event)
{
    if (event == SOL_LWM2M_BOOTSTRAP_EVENT_FINISHED) {
        ...
        return;
    } else if (event == SOL_LWM2M_BOOTSTRAP_EVENT_WRITE) {
        printf("Bootstrap Write received! Object(s) <obj>/<objInst> created!\n");
        return;
    } else if (event == SOL_LWM2M_BOOTSTRAP_EVENT_DELETE) {
        printf("Bootstrap Delete received! Object <obj>/<objInst> deleted!\n");
        return;
    }
...
main(int argc, char *argv[])
{
    ...
    if (!data_ctx.is_bootstrap) {
        ...
        r = sol_lwm2m_client_add_bootstrap_monitor(client, bootstrap_cb,
            NULL);
        if (r < 0) {
